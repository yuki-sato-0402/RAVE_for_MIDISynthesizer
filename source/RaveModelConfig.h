...
static std::vector<anira::ModelData> getModelData()
{
    ...
    // Expected symbols (examples): BinaryData::DistortedGTV1_e02d7ebfe3_streaming_norm_ts
    ...
    if (!fs::exists(modelPath))
    {
        ...
        if (ofs)
        {
            ofs.write(reinterpret_cast<const char*>(BinaryData::DistortedGTV1_e02d7ebfe3_streaming_norm_ts),
                      (std::streamsize)BinaryData::DistortedGTV1_e02d7ebfe3_streaming_norm_tsSize);
            ofs.close();
        }
    }
...
#else
    return {{RAVE_MODEL_DIR + std::string("DistortedGTV1_e02d7ebfe3_streaming_norm.ts"), anira::InferenceBackend::CUSTOM}};
#endif
}
